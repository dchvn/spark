...
class DStream(Generic[T]):
    ...
    def __init__(
        ...
        self._ssc = ssc
        self._sc: SparkContext = ssc._sc  # type: ignore[attr-defined]
    ...
    def foreachRDD(
        ...
        if func.__code__.co_argcount == 1:
            old_func = cast(Callable[["RDD[T]"], None], func)
            func = lambda t, rdd: old_func(rdd)
        jfunc = TransformFunction(self._sc, func, self._jrdd_deserializer)
        api = self._ssc._jvm.PythonDStream  # type: ignore[attr-defined]
    ...
    def checkpoint(self, interval: int) -> "DStream[T]":
        ...
        self._jdstream.checkpoint(self._ssc._jduration(interval))  # type: ignore[attr-defined]
        return self
    ...
    def countByValue(self) -> "DStream[Tuple[T, int]]":
        ...
        """
        return (
            self.map(lambda x: (x, 1))  # type: ignore[return-value]
    ...
    def saveAsTextFiles(self, prefix: str, suffix: Optional[str] = None) -> None:
        ...
        """
        def saveAsTextFile(t: datetime, rdd: "RDD[T]") -> None:
    ...
    @overload
    def transform(
            self, func: Callable[[datetime, "RDD[T]"], "RDD[U]"]
    ...
    def transform(
        ...
        """
        if func.__code__.co_argcount == 1:
            oldfunc = cast(Callable[["RDD[T]"], "RDD[U]"], func)
    ...
    def transformWith(
        ...
        if func.__code__.co_argcount == 2:
            oldfunc = cast(Callable[["RDD[T]", "RDD[U]"], "RDD[V]"], func)
        ...
        dstream = self._sc._jvm.PythonTransformed2DStream(  # type: ignore[attr-defined]
            self._jdstream.dstream(),
            other._jdstream.dstream(),  # type: ignore[attr-defined]
            jfunc
        )
        jrdd_serializer = (
            self._jrdd_deserializer
            if keepSerializer else self._sc.serializer  # type: ignore[attr-defined]
        )
    ...
    def union(self, other: "DStream[U]") -> "DStream[Union[T, U]]":
        ...
        return self.transformWith(
            lambda a, b: a.union(b), other, True)  # type: ignore[call-overload]
    ...
    def cogroup(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.cogroup(b, numPartitions), other)  # type: ignore[call-overload]
    ...
    def join(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.join(b, numPartitions), other)  # type: ignore[call-overload]
    ...
    def leftOuterJoin(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.leftOuterJoin(b, numPartitions), other)  # type: ignore[call-overload]
    ...
    def rightOuterJoin(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.rightOuterJoin(b, numPartitions), other)  # type: ignore[call-overload]
    ...
    def fullOuterJoin(
        ...
        if numPartitions is None:
            numPartitions = self._sc.defaultParallelism
        return self.transformWith(
            lambda a, b: a.fullOuterJoin(b, numPartitions), other)  # type: ignore[call-overload]
    ...
    def _jtime(self, timestamp: Union[datetime, int, float]) -> JavaObject:
        ...
        return self._sc._jvm.Time(int(timestamp * 1000))  # type: ignore[attr-defined]

    def slice(
        self, begin: Union[datetime, int], end: Union[datetime, int]
    ) -> List["RDD[T]"]:
        """
    ...
    def window(
        self, windowDuration: int, slideDuration: Optional[int] = None
    ) -> "DStream[T]":
        """
        ...
        d = self._ssc._jduration(windowDuration)  # type: ignore[attr-defined]
        if slideDuration is None:
            return DStream(self._jdstream.window(d), self._ssc, self._jrdd_deserializer)
        s = self._ssc._jduration(slideDuration)  # type: ignore[attr-defined]
    ...
    def reduceByWindow(
        ...
        reduced = keyed.reduceByKeyAndWindow(
            reduceFunc,  # type: ignore[arg-type]
            invReduceFunc,  # type: ignore[arg-type]
            windowDuration,
            slideDuration,
            1
        )
    ...
    def countByValueAndWindow(
        ...
        return counted.filter(lambda kv: kv[1] > 0)  # type: ignore[return-value]
    ...
    def reduceByKeyAndWindow(
        ...
        if invFunc:
            def reduceFunc(t: datetime, a: DStream, b: DStream) -> DStream:
            ...
            def invReduceFunc(t: datetime, a: DStream, b: DStream) -> DStream:
                b = b.reduceByKey(func, numPartitions)
            ...
            dstream = self._sc._jvm.PythonReducedWindowedDStream(  # type: ignore[attr-defined]
                reduced._jdstream.dstream(),
                jreduceFunc, jinvReduceFunc,
                self._ssc._jduration(windowDuration),  # type: ignore[attr-defined]
                self._ssc._jduration(slideDuration))  # type: ignore[attr-defined]
            return DStream(
                dstream.asJavaDStream(),
                self._ssc,
                self._sc.serializer  # type: ignore[attr-defined]
            )
        else:
    ...
    def updateStateByKey(
        ...
        intNumPartitions = cast(int, numPartitions)
        ...
        def reduceFunc(t: datetime, a: DStream, b: DStream) -> DStream:
            if a is None:
                g = b.groupByKey(intNumPartitions).mapValues(lambda vs: (list(vs), None))
            else:
                g = a.cogroup(b.partitionBy(intNumPartitions), intNumPartitions)
                g = g.mapValues(lambda ab: (list(ab[1]), list(ab[0])[0] if len(ab[0]) else None))
            state = g.mapValues(
                lambda vs_s: updateFunc(vs_s[0], vs_s[1]))  # type: ignore[arg-type]
        ...
        if initialRDD:
            initialRDD = cast("RDD[Tuple[K, S]]", initialRDD)
            initialRDD = initialRDD._reserialize(  # type: ignore[attr-defined]
                self._jrdd_deserializer)
            initialRDD = cast("RDD[Tuple[K, S]]", initialRDD)
            dstream = self._sc._jvm.PythonStateDStream(  # type: ignore[attr-defined]
                self._jdstream.dstream(),
                jreduceFunc,
...
class TransformedDStream(DStream[U]):
    ...
    @overload
    def __init__(
        self, prev: DStream[T], func: Callable[["RDD[T]"], "RDD[U]"]
    ):
        ...
    ...
    def __init__(
        ...
        self._ssc = prev._ssc
        self._sc = self._ssc._sc  # type: ignore[attr-defined]
        self._jrdd_deserializer = self._sc.serializer  # type: ignore[attr-defined]
        self.is_cached = False
        self.is_checkpointed = False
    ...
    def _jdstream(self) -> JavaObject:
        ...
        dstream = self._sc._jvm.PythonTransformedDStream(  # type: ignore[attr-defined]
            self.prev._jdstream.dstream(), jfunc)
